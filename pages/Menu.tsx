...
import { X, ZoomIn, Sparkles, Tag, AlertCircle, ChevronUp, Layers, TrendingDown, CheckCircle2, PlusCircle, Plus, Utensils, Maximize, Zap } from 'lucide-react';
import { MenuItem } from '../types';

// Helper function to render description text while HIDDING legacy bracketed info
const formatDescription = (text: string) => {
  if (!text) return null;
  
  // Remove all content within square brackets [like this] as they are now handled by badges
  ...
  return (
    <div className="flex flex-col gap-1">
      {cleanedText.split('\n').map((line, i) => (
        line.trim() ? <span key={i}>{line}</span> : null
      ))}
    </div>
  );
};

// Component for a unified Badge (Icon + Text) - Styled based on the requested image
const OptionBadge: React.FC<{ icon: React.ReactNode; text: string; isGold?: boolean; isNew?: boolean }> = ({ icon, text, isGold = true, isNew = false }) => (
    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border transition-all duration-300 relative ${
        isGold 
        ? 'border-gold/40 bg-black/40 text-gold shadow-[0_0_15px_rgba(212,175,55,0.1)]' 
        : 'border-white/20 bg-white/5 text-gray-300'
    } ${isNew ? 'animate-badge-burst' : ''}`}>
        <span className="shrink-0 opacity-80">{icon}</span>
        <span className="text-[9px] font-black uppercase tracking-widest whitespace-nowrap">{text}</span>
        
        {isNew && (
           <div className="absolute inset-0 rounded-full border border-gold animate-ping-slow pointer-events-none"></div>
        )}
    </div>
);
...
const MenuGridItem: React.FC<MenuGridItemProps> = ({ item, lang, primaryColor, onImageClick }) => {
  ...
  // Split topping text if it contains "/" or "\n" to handle multiple items
  const toppingInput = item.toppingText?.[lang] || '';
  const toppings = toppingInput ? toppingInput.split(/[/|\n]/).map(t => t.trim()).filter(Boolean) : [];
  ...
  return (
    <div className={`flex gap-6 group transition-opacity duration-300 ${isSoldOut ? 'opacity-60' : 'opacity-100'}`}>
      ...
      >
        ...
        {isSoldOut && (
           <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <span className="text-[10px] md:text-xs font-black text-white bg-red-600 px-4 py-2 tracking-widest border border-white/20">
                SOLD OUT
              </span>
           </div>
        )}
        ...
        {hasDiscount && !isSoldOut && (
           <div className="absolute top-0 left-0 bg-gold text-black text-[9px] font-bold px-1.5 py-0.5 tracking-tighter z-10">
              SAVE RM{savings}
           </div>
        )}

        {item.isNew && !isSoldOut && (
           <div className="absolute bottom-0 right-0 bg-gold text-black text-[9px] font-black px-2 py-1 tracking-widest z-10 animate-pulse">
              NEW
           </div>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col justify-between py-1">
        <div>
          <div className="flex justify-between items-start mb-2 border-b border-neutral-800 pb-3 border-dashed">
            <div className="flex flex-col gap-2.5 flex-1 pr-4">
                <div className="flex items-center gap-3 flex-wrap">
                    <h3 className={`text-lg md:text-xl font-bold uppercase transition-colors leading-tight ${isSoldOut ? 'text-gray-500' : 'text-white cursor-pointer hover:text-gold'}`} onClick={() => !isSoldOut && onImageClick(item)}>
                        {item.name[lang]}
                    </h3>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {item.isNew && !isSoldOut && (
                        <OptionBadge icon={<Zap size={10} className="fill-gold" />} text={lang === 'en' ? 'NEW' : '신메뉴'} isNew={true} />
                      )}

                      {item.isPopular && !isSoldOut && (
                        <OptionBadge icon={<Sparkles size={10} />} text={lang === 'en' ? 'BEST' : '베스트'} isGold={false} />
                      )}

                      {item.hasExtraNoodles && !isSoldOut && (
                        <OptionBadge icon={<Utensils size={10} />} text={lang === 'en' ? 'NOODLE+' : '면 추가'} />
                      )}

                      {item.hasSizeUp && !isSoldOut && (
                        <OptionBadge icon={<Maximize size={10} />} text={lang === 'en' ? 'SIZE UP' : '사이즈 업'} />
                      )}
                    </div>
                </div>

                {/* Toppings Badges Row - Multi-item support */}
                {item.hasToppings && !isSoldOut && toppings.length > 0 && (
                  <div className="flex gap-2 flex-wrap mt-0.5">
                      {toppings.map((t, idx) => (
                        <OptionBadge key={idx} icon={<PlusCircle size={10} />} text={t} />
                      ))}
                  </div>
                )}
            </div>
          ...
          <div className={`text-sm leading-relaxed ${isSoldOut ? 'text-gray-600' : 'text-gray-400'}`}>
            {formatDescription(item.description[lang])}
          </div>
        </div>
      </div>
    </div>
...
const Menu: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-texture-paper pt-24 pb-32 relative">
      <style>{`
        @keyframes badge-burst {
          0% { transform: scale(1); box-shadow: 0 0 0 0 rgba(212, 175, 55, 0.4); }
          50% { transform: scale(1.05); box-shadow: 0 0 20px 5px rgba(212, 175, 55, 0.2); }
          100% { transform: scale(1); box-shadow: 0 0 0 0 rgba(212, 175, 55, 0.4); }
        }
        .animate-badge-burst {
          animation: badge-burst 2s infinite ease-in-out;
        }
        @keyframes ping-slow {
      ...
      {/* Floating Mobile Nav */}
      {isTouch && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] flex flex-col items-center gap-2 animate-fade-in">
          <button 
            onClick={() => setShowMobileNav(true)}
            className="flex items-center gap-3 px-6 py-4 rounded-full bg-black/80 backdrop-blur-xl border border-gold/30 shadow-2xl transition-all active:scale-95 group"
          >
            ...
            <div className="relative w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded shadow-2xl overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="w-full bg-black relative shrink-0 flex items-center justify-center min-h-[250px] overflow-hidden">
                   <img 
                    ...
                    className={`w-full h-auto max-h-[50vh] object-contain transition-transform duration-1000 ${selectedItem.isSoldOut ? 'grayscale blur-md opacity-40' : 'scale-100'}`} 
                   />
                    ...
                    <div className="flex flex-col items-center gap-4">
                         <div className="flex flex-col items-center gap-1">
                            <div className="flex items-center gap-2 flex-wrap justify-center">
                                <h3 className={`text-xl md:text-3xl font-black uppercase tracking-tight leading-tight ${selectedItem.isSoldOut ? 'text-gray-500' : 'text-white'}`}>
                         ...
                         {/* Options Summary In Modal */}
                         {(selectedItem.isNew || selectedItem.hasToppings || selectedItem.hasExtraNoodles || selectedItem.hasSizeUp) && !selectedItem.isSoldOut && (
                            <div className="flex flex-wrap justify-center gap-2 mt-1">
                               {selectedItem.isNew && <OptionBadge icon={<Zap size={10} className="fill-gold" />} text={lang === 'en' ? 'NEW' : '신메뉴'} isNew={true} />}
                               {selectedItem.hasToppings && (
                                   (selectedItem.toppingText?.[lang] || '').split(/[/|\n]/).map((t, idx) => (
                                     <OptionBadge key={idx} icon={<PlusCircle size={10} />} text={t.trim()} />
                                   ))
                               )}
                               {selectedItem.hasExtraNoodles && <OptionBadge icon={<Utensils size={10} />} text={lang === 'en' ? 'NOODLE+' : '면 추가'} />}
                               {selectedItem.hasSizeUp && <OptionBadge icon={<Maximize size={10} />} text={lang === 'en' ? 'SIZE UP' : '사이즈 업'} />}
                            </div>
                         )}
                         ...
                         <div className="flex flex-col items-center w-full pt-4 border-t border-white/5">
                            ...
                            <div className="flex items-baseline gap-2">
                                ...
                                <span className={`text-4xl md:text-5xl font-black whitespace-nowrap drop-shadow-2xl ${selectedItem.isSoldOut ? 'text-gray-600' : ''}`} style={{ color: selectedItem.isSoldOut ? undefined : primaryColor }}>
          ...
          <div className="flex-1">
             <div className="animate-fade-in">
                {categories.sort((a,b) => a.order - b.order).map(cat => {
                   ...
                   return (
                      <div key={cat.id} id={`cat-${cat.id}`} ref={el => { categoryRefs.current[cat.id] = el; }} className="mb-24 last:mb-0 pt-8">
                         ...
                         {catSubCategories.map(sub => {
                            const subItems = menu.filter(item => item.subCategoryId === sub.id).sort((a, b) => (a.order ?? 9999) - (b.order ?? 9999));
                            ...
                            return (
                               <div key={sub.id} className="mb-16 last:mb-0">
                                  ...
                                  <div className="grid grid-cols-1 gap-12 lg:gap-16">
                                     {subItems.map(item => (
                                       <MenuGridItem key={item.id} item={item} lang={lang} primaryColor={primaryColor} onImageClick={setSelectedItem} />
                                     ))}
                                  </div>
                               </div>
                            );
