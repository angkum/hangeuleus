    ...
    <div className="min-h-screen bg-neutral-950 pt-24 pb-24 relative">
      <style>{`
        @media (min-width: 1024px) {
            .desktop-reset-bg { background-color: transparent !important; }
            .desktop-active-bg { background-color: rgba(255, 255, 255, 0.05) !important; }
        }
      `}</style>
      
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-sm p-4 animate-fade-in" onClick={closeModal}>
            <button onClick={closeModal} className="absolute top-4 right-4 text-white/50 hover:text-white z-[110] bg-black/40 rounded-full p-1"><X size={28} /></button>
            <div className="relative w-full max-w-lg md:max-w-xl bg-neutral-900 border border-neutral-800 rounded-sm shadow-2xl overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="w-full bg-black relative shrink-0 flex items-center justify-center min-h-[200px]">
                   <img src={selectedItem.image} alt={selectedItem.name[lang]} className={`w-full h-auto max-h-[60vh] object-contain ${selectedItem.isSoldOut ? 'grayscale blur-sm opacity-50' : ''}`} />
                   {selectedItem.isSoldOut && (
                       <div className="absolute inset-0 flex items-center justify-center">
                           <div className="bg-red-600 text-white px-8 py-4 font-black text-2xl shadow-2xl tracking-[0.2em] transform -rotate-12 border-4 border-white">SOLD OUT</div>
                       </div>
                   )}
                   {!!(selectedItem.originalPrice && selectedItem.originalPrice > selectedItem.price) && !selectedItem.isSoldOut && (
                        <div className="absolute bottom-4 right-4 bg-gold text-black px-4 py-2 font-black text-sm shadow-xl tracking-widest animate-pulse">SAVE RM{selectedItem.originalPrice - selectedItem.price}</div>
                    )}
                </div>
                <div className="p-6 overflow-y-auto bg-neutral-900 border-t border-white/5">
                    <div className="flex justify-between items-start gap-3 mb-2">
                         <div className="flex flex-col gap-1">
                            ...
                            {selectedItem.isPopular && !selectedItem.isSoldOut && <span className="text-gold text-[10px] uppercase tracking-widest font-bold">{lang === 'en' ? 'Most Popular' : '한그릇 인기 메뉴'}</span>}
                            {selectedItem.isSoldOut && <span className="text-red-500 text-[10px] uppercase tracking-widest font-bold flex items-center gap-1"><AlertCircle size={10} />{lang === 'en' ? 'Unavailable' : '현재 주문 불가'}</span>}
                         </div>
                        <div className="flex flex-col items-end">
                            {!!(selectedItem.originalPrice && selectedItem.originalPrice > selectedItem.price) && !selectedItem.isSoldOut && (
                    ...
                    <p className={`text-sm leading-relaxed mt-4 border-l pl-4 ${selectedItem.isSoldOut ? 'text-gray-600 border-gray-800' : 'text-gray-400 border-gold/30'}`}>{selectedItem.description[lang]}</p>
                </div>
            </div>
        </div>
      )}
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          <aside className="lg:w-1/4 lg:shrink-0 relative z-30">
             <div className="sticky top-[80px] lg:top-24 z-30 bg-neutral-950/95 backdrop-blur-md lg:backdrop-blur-none lg:bg-transparent -mx-4 px-4 py-4 lg:mx-0 lg:px-0 lg:py-0 border-b border-neutral-800 lg:border-b-0 lg:border-r lg:border-neutral-800 lg:pr-8 h-auto lg:h-[calc(100vh-6rem)] overflow-y-auto">
                <h2 className="text-2xl font-serif text-white mb-6 hidden lg:block border-b border-gold pb-4">{lang === 'en' ? 'Category' : '카테고리'}</h2>
                <div className="flex lg:flex-col gap-3 lg:gap-2 overflow-x-auto pb-1 lg:pb-0 scrollbar-hide">
                  {categories.sort((a,b) => a.order - b.order).map(cat => {
                    const isActive = activeCategoryId === cat.id;
                    return (
                        <button key={cat.id} onClick={() => { setActiveCategoryId(cat.id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`whitespace-nowrap px-6 py-3 lg:px-4 lg:py-3 text-sm md:text-base uppercase tracking-widest text-left transition-all duration-300 rounded-full lg:rounded-none border lg:border-0 lg:border-l-4 flex-shrink-0 ${isActive ? 'text-black font-extrabold shadow-lg lg:text-white lg:shadow-none desktop-active-bg' : 'bg-neutral-900 text-gray-400 border-neutral-800 hover:border-gray-600 lg:bg-transparent lg:border-transparent lg:hover:text-gray-300 lg:hover:bg-white/5 desktop-reset-bg'}`} style={{ backgroundColor: isActive ? primaryColor : undefined, borderColor: isActive ? primaryColor : undefined, borderLeftColor: isActive && window.innerWidth >= 1024 ? primaryColor : undefined }}>{cat.name[lang]}</button>
                    );
                  })}
                </div>
             </div>
                   ...
                   return (
                      <div key={sub.id} className="mb-16 last:mb-0">
                         <div className="flex items-center gap-4 mb-8">
                            ...
                            <h3 className="text-xl md:text-2xl font-bold text-gold uppercase tracking-widest text-center px-4">{sub.name[lang]}</h3>
                            <div className="h-px bg-neutral-800 flex-1"></div>
                         </div>
                         <div className="grid grid-cols-1 gap-12">
                            {subItems.map(item => <MenuGridItem key={item.id} item={item} lang={lang} primaryColor={primaryColor} onImageClick={setSelectedItem} />)}
                         </div>
                      </div>
                   )
                })}
